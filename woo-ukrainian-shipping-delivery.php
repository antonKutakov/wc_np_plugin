...
/**
* Plugin Name: Shipping via Ukrainian shipping services for WooCommerce
* Description: Adds a Ukrainian shipping services to Woocommerce
...
define("WUSD_PATH", plugin_dir_url( __FILE__ ));
define("WUSD_DOMAIN", 'woo-ukrainian-shipping-delivery');
define("WUSD_FILE_PREFIX", "wusd");

require plugin_dir_path( __FILE__ ) . 'vendor/autoload.php';

use WooUkrainianShippingDelivery\App\DB\WUSD_DB;
use WooUkrainianShippingDelivery\App\Loader;
...
function remove_base_tables_deactivation(){
    WUSD_DB::getInstance()->drop_table("novaposhta_areas")->drop_table("novaposhta_cities")->drop_table("novaposhta_warehouses");
}
...
if( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ){
    ...
    require_once plugin_dir_path( __FILE__ ) . 'app/woocommerce-extensions/nova-poshta/wusd-np-wc-shipping.php';
}
else{
    add_action( 'admin_notices', 'wusd_without_woo_notice' );

    function wusd_without_woo_notice() {
        ?>
...
function base_init(){
    (new Loader())->wusd_load_dependencies();
}
